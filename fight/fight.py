import numpy as np
import copy,os
from utils.draw import draw_chess
class Fight:
    '''
    '''
    ...
    def _assign_hexes(self,hexes,mynum,myarr,myitems,mysyn,myinfo,myskill,
        ...
        for mn,ma,mitem,minf,on,oa,oitem,oinf in \
            ...
            for c,(m,o) in enumerate(zip(mitem,oitem)):
                hexes[ma[0],ma[1],12+c] = m
                hexes[oa[0],oa[1],12+c] = o
        return hexes
    ...
    def _fight_tic(self,hexes,n,draw=False):
        ...
        for oa,ma in zip(self.opparr,self.myarr):
            ...
            hexes,is_attack = self._one_champ_tic(hexes,oar,oa,-1,1,oa_enemies,tic)
            hexes,is_attack = self._one_champ_tic(hexes,mar,ma,1,-1,ma_enemies,tic)
        skill = None
