...
class Skill:
    ...
    def _leona(self,level,enemies,shield=[40,80,120],stop=False):
        if stop:
            ...
            self.hexes[self.arr[0],self.arr[0],8] -= shield[level]
        else:
            self.hexes[self.arr[0],self.arr[0],7] += shield[level]
            self.hexes[self.arr[0],self.arr[0],8] += shield[level]
            self.hexes[self.arr[0],self.arr[0],26] = 9
    ...
    def _jarvan_iv(self,level,enemies,speed=[0.5,0.75,1],stop=False):
        if stop:
            ...
            for tx,ty in targets:
                self.hexes[tx,ty,6] += speed[level]
                self.jarvan.append(self.hexes[tx,ty,-1])
            self.hexes[self.arr[0],self.arr[1],26] = 7
